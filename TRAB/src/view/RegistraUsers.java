	...
	public void incluirCliente() {
		if (validacaoCampos()) {
			if (listaClientes.size() != 0) {
				...
				listaClientes
						.add(new User(cliente.getId() + 1, TxtCpf.getText(), Txtemail.getText(), TxtName.getText()));
			} else {
		...
		if (validacaoCampos()) {
			...
			for (User cliente : lista) {
				if (cliente.getId().equals(Integer.parseInt(Txtid.getText()))) {
					...
					listaClientes.add(new User(Integer.parseInt(Txtid.getText()), TxtCpf.getText(), Txtemail.getText(),
							TxtName.getText()));
				}
			}
